-- ==========================================================
-- TEST: Allocation Blocked — Status not AV
-- Verifies that units with a non-AV status (e.g. BL = blocked,
-- AL = allocated) are NOT considered for new allocation.
--
-- Setup:
--   Pallet A — PTW / BL (blocked)      — must be ignored
--   Pallet B — PTW / AV (available)    — must be allocated
--
...
BEGIN TRY
    ...
    VALUES
        ('TEST-STATUS-SKU', 'Test Status Blocked SKU', '09999900000701', 'Case',
    ...
    VALUES ('_TEST-CUST-STATUS', 'Test Customer STATUS', 'Test Customer', 'GB', 1, SYSUTCDATETIME());
    SET @CustomerId = SCOPE_IDENTITY();
    DECLARE @OrderId INT;

    INSERT INTO outbound.outbound_orders (order_ref, customer_party_id, order_status_code, required_date)
    VALUES ('TEST-ORD-STATUS', @CustomerId, 'NEW', CAST(GETDATE() AS DATE));
    ...
    -- Pallet A (BL) must NOT be allocated
    IF EXISTS (
        SELECT 1 FROM outbound.outbound_allocations a
        WHERE a.inventory_unit_id = @UnitA AND a.allocation_status = 'PENDING'
    )
        RAISERROR('STATUS BLOCKED TEST FAILED: Blocked (BL) pallet was incorrectly allocated.', 16, 1);

    -- Pallet B (AV) must be allocated
    IF NOT EXISTS (
        SELECT 1 FROM outbound.outbound_allocations a
        WHERE a.inventory_unit_id = @UnitB AND a.allocation_status = 'PENDING'
    )
        RAISERROR('STATUS BLOCKED TEST FAILED: Available (AV) pallet was not allocated.', 16, 1);

    PRINT 'TEST PASSED: Allocation blocked status — only AV unit allocated.';
